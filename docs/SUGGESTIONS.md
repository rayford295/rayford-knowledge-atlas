...
## 2026-06 Improvement Batch (status)

A second review addressed the following:

- **Licensing** — Added `LICENSE` (MIT, code) and `LICENSE-CONTENT.md` (CC BY 4.0, content), plus README sections. The repo can now be forked legally.
- **Nav de-duplication** — The five shared top-level nav links are now generated by `scripts/build-html.js` between `<!-- NAV:START/END -->` markers. Page-specific tail links (Reading Vault, Paper Vault, Return to Graph) stay inline.
- **Cache-bust unification** — `build-html.js` stamps a single content-hash `?v=` across all pages, replacing the drifted manual version strings. CI workflows now commit the re-stamped HTML.
- **`verify-atlas.js`** — Now validates that every node has a numeric position and warns when a node falls outside the 0–900 / 0–700 canvas.
- **`fetch-scholar.js`** — Added 3-attempt exponential backoff (2s/4s/8s) before falling back to the previous snapshot. *(Superseded 2026-08-22: the backoff did not help. The failure was a per-IP 403, not a rate limit, so all three attempts hit the same block. See the 2026-08 batch below.)*
- **XSS hardening** — `script.js` now escapes external/free-text fields before interpolating them into `innerHTML`.
- **Tooling** — Added `.editorconfig`, `.prettierrc.json`, `.prettierignore`, `package-lock.json`, and `format` / `format:check` scripts.
- **Content** — Added a `firebridge` placeholder node so connections can be tracked while the paper develops (body is TODO scaffolding).
- **Ambient animation (3.x / 4.3)** — Reviewed and intentionally left unchanged. The floating-node + particle motion is a deliberate aesthetic, and browsers throttle `requestAnimationFrame` in background tabs, so the idle-CPU concern is limited.

## 2026-08 Improvement Batch (status)

- **Scholar refresh reliability** — The weekly workflow had been green every week
  while the snapshot sat frozen for 19 days: Scholar 403s GitHub's datacenter IPs,
  and `fetch-scholar.js` keeps the previous snapshot and exits 0 on failure. Fixed
  by moving the retry from inside a job to across jobs (a 6-hourly catch-up gated
  on snapshot age, since each Actions job gets a fresh runner IP), skipping the
  rebuild and commit when the snapshot did not change, failing the primary run once
  the snapshot passes 14 days, and stamping the snapshot date under the citation
  count on `papers.html`. See the Scholar Snapshot Rules in `CLAUDE.md`.
- **Curated coverage** — Added `beacon`, `rapid`, `geosteward`, and
  `vgi-spatial-bias`. The `rapid` page replaced its own auto-generated Scholar
  stub, so Scholar-only records fell from 6 to 5 while curated pages rose 17 to 21.
- **Backlog made explicit** — Remaining uncovered work, half-connected bridge
  questions, and open editorial decisions now live in `docs/ATLAS_BACKLOG.md`
  instead of being rediscovered each time someone notices the site looks stale.
